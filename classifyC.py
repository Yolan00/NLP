...
def read_category_vectors():
    ...
    with open('/home/yolan00/Desktop/nlp/data/category_vectors.txt', 'r') as f:
        for l in f:
            l = l.rstrip('\n')
            fields = l.split()
            cat = fields[0].replace('_', ' ')  # Convert category name format
            vec = np.array([float(v) for v in fields[1:]])
            vectors[cat] = vec
    return vectors

def get_combined_ngrams(text, ngram_sizes):
    text = text.lower()
    combined_ngrams = {}
    for n in ngram_sizes:
        for i in range(0, len(text) - n + 1):
            ngram = text[i:i + n]
            if ngram in combined_ngrams:
                combined_ngrams[ngram] += 1
            else:
                combined_ngrams[ngram] = 1
...
def classify_queries(ngram_sizes, ground_truth):
    vocab = read_vocab()
    ...
    for q in queries:
        combined_ngrams = get_combined_ngrams(q, ngram_sizes)
        total_ngrams = sum(combined_ngrams.values())
        combined_ngrams = normalise_tfs(combined_ngrams, total_ngrams)
        qvec = mk_vector(vocab, combined_ngrams)
    ...
    with open("classification_results_combined.csv", "w", newline='') as csvfile:
    ...
    with open("true_vs_predicted_combined.csv", "w", newline='') as csvfile:
        writer = csv.writer(csvfile)
...
if __name__ == "__main__":
    ...
    ngram_sizes = [2, 3, 4, 5, 6, 7]  # Use n-gram sizes 2 to 7
    classify_queries(ngram_sizes, ground_truth)
